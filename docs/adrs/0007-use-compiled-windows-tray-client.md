...
The NomadInbox tray started as a PowerShell WinForms script. That kept the bootstrap fast, but it made tray menu interactions easy to block because status refresh and sync calls lived near UI event handlers.
...
The tray client reads cached status first and performs HTTP refresh, Sync now, and auto-sync start/stop through background tasks. The menu renders only from cached in-memory state, so opening the tray menu does not wait on HTTP, provider sync, or CLI execution.
...
- The compiled executable is generated under ignored `target/` and should not be committed.
- PowerShell remains as a launcher/build wrapper and service layer, not as the tray UI runtime.
- Future tray changes must preserve the nonblocking menu rule: no HTTP, sync, or CLI calls during menu construction.
