...
| Delete safety | Trash/delete attempts with double explicit confirmation / trash-delete attempts |
...
| Tray popup responsiveness | Tray popup opens from cached status without waiting on HTTP, sync, or CLI calls |
...
| Delete confirmation | 100% of trash/delete actions require double explicit confirmation |
...
| Compiled tray client | Compact native popup exposes Sync now, auto-sync toggle, per-account status, Settings, and runtime folder actions without blocking on refresh | Logs, errors, provider diagnostics, locale, and time-zone details stay in Settings |
| Tray popup latency | 95% of tray popup opens complete within 300 ms from cached state | HTTP refresh and sync run asynchronously after the popup is usable |
