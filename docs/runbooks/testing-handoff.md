...
| Windows helper | Helper install without mail access | `install windows-helper --data-dir <temp> --install-root <temp>` and regular install with `--start-tray --register-startup --show-popup` | Initializes helper/status paths; regular install can start tray and register current-user startup; neither path reads mail or enables auto sync. |
